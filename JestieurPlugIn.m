...
- (void) MCGestureDelegateGestureNotRecognized:(MCGestureView *) _view {
	...
	self.outputScaleX = 0.0;
	self.outputScaleY = 0.0;
	self.outputName = @"";
...
- (void) MCGestureDelegate:(MCGestureView *) _view recognizedGestureWithName:(NSString *) _name score:(CGFloat) _score ratio:(CGFloat) _ratio rotation:(CGFloat) _rotation size:(CGSize) _size
{
	...
	self.outputScaleX = _size.width;
	self.outputScaleY = _size.height;
}
...
@dynamic outputScaleX, outputScaleY;
...
+ (NSDictionary*) attributesForPropertyPortWithKey:(NSString*)key
{
	...
	if([key isEqualToString:@"outputLastUnistroke"])
		return [NSDictionary dictionaryWithObjectsAndKeys:
				...
				nil];
	if([key isEqualToString:@"outputScaleX"])
		return [NSDictionary dictionaryWithObjectsAndKeys:
				@"X Scale", QCPortAttributeNameKey,
				nil];
	if([key isEqualToString:@"outputScaleY"])
		return [NSDictionary dictionaryWithObjectsAndKeys:
				@"Y Scale", QCPortAttributeNameKey,
				nil];
